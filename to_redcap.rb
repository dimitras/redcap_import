...
Dir["collos_exports/#{subj_id}/*.csv"].each do |ifile|
	...
	# output
	ofile = ifile.split("/")[-1]
	Dir.mkdir("to_import/#{subj_id}") unless File.exists?("to_import/#{subj_id}")
	CSV.open("to_import/#{subj_id}/import_#{ofile}", "w") do |csv|
